...
def softmax(x):
    ...
    ### YOUR CODE HERE
    _max = tf.reduce_max(x,axis=x.get_shape().ndims-1,keep_dims=True)
    _den = tf.reduce_sum(tf.exp(x-_max),axis=x.get_shape().ndims-1,keep_dims=True)
    out = tf.exp(x-_max)/_den
    ### END YOUR CODE
