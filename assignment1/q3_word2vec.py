...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    V,D = outputVectors.shape #vocabulary size
    y = np.zeros(V)
    y[target] = 1
    dot_product = np.dot(predicted,outputVectors.T)
    y_hat = softmax(dot_product) # probability = y_hat
    cost = -np.sum(y*np.log(y_hat)) #cross entropy cost
    gradPred = np.dot(y_hat, outputVectors) - outputVectors[target]
    grad  = np.dot((y_hat - y).reshape(V,1),predicted.reshape(1,D))
#    V, D     = outputVectors.shape
#    y = np.zeros(V)
#    y[target] = 1
#    dot_product = np.dot (predicted.reshape(1,D),outputVectors.reshape(D,V))
#    y_hat = softmax(dot_product)
#    cost = -np.sum(y*np.log(y_hat)) #cross entropy cost
#    gradPred = np.dot(y_hat.reshape(1,V),outputVectors.reshape(V,D)) - outputVectors[target].reshape(1,D)
#    grad = predicted.reshape(1,D) *  (y_hat - y).T
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    centerWordVector = inputVectors[centerWord_idx]
    for context in contextWords:
        contextWord_idx = tokens[context]
        cost_, gradIn_, gradOut_ = \
        word2vecCostAndGradient(centerWordVector,contextWord_idx,outputVectors,dataset)
        cost += cost_
        gradIn[centerWord_idx] += gradIn_
        gradOut += gradOut_
    ### END YOUR CODE
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    for contextWord in contextWords:
        contextWord_idx = tokens[contextWord]
        contextWordVector = inputVectors[contextWord_idx]
        cost_, gradIn_, gradOut_ = \
        word2vecCostAndGradient(contextWordVector, centerWord_idx, outputVectors, dataset)
        cost += cost_
        gradIn[contextWord_idx] += gradIn_
        gradOut += gradOut_
    ### END YOUR CODE
